...
class CareerChangeReportGenerator {
    ...
    async generateReport() {
        const reports = new Map<string, RoleCareerChangeReport>();
        ...
        TeamMemberLoader.loadTeamMembersAsMap(this.#includeExternal)
            .forEach(it => {
                const roleName = it.name!;
                reports.set(it.id!, new RoleCareerChangeReport(roleName));
            });
        (await CareerChangeLogStorage.getInstance().loads())
            .filter(it => it.roleId !== undefined)
            .filter(it => reports.has(it.roleId!))
            .forEach(it => {
                reports.get(it.roleId!)?.logList.push(it);
    ...
    #generate(candidates: CareerChangeLog[]) {
        const roles = new Map<string, RoleCareerChangeReport>();
        const memberIds = TeamMemberLoader.loadTeamMembers()
            .filter(it => this.#includeExternal || !it.external)
            .map(it => it.id!);
        TeamMemberLoader.loadTeamMembers()
            ...
            .forEach(config => {
                roles.set(config.id!, new RoleCareerChangeReport(config.name!));
            });
