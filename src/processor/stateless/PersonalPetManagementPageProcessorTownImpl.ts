...
class PersonalPetManagementPageProcessorTownImpl extends PersonalPetManagementPageProcessor {
    ...
    async doInitialize(credential: Credential, context?: PageProcessorContext): Promise<void> {
        ...
        const locationMode = new LocationModeTown(townId);
...
function doProcess(credential: Credential, petList: Pet[], studyStatus: number[], peopleFinder: PeopleFinderComponent) {
    ...
    new PersonalStatus(credential).load().then(role => {
        ...
        new PersonalEquipmentManagement(credential).open().then(page => {
            ...
            doRender(credential, petList, studyStatus, peopleFinder, role);
            if (cage) {
                $("#openCageButton").show();
...
function doRender(credential: Credential, petList: Pet[], studyStatus: number[],
                  peopleFinder: PeopleFinderComponent, role?: Role) {
...
function doRefresh(credential: Credential, peopleFinder: PeopleFinderComponent) {
    new PersonalPetManagement(credential).open().then(petPage => {
        ...
        new PersonalStatus(credential).open().then(page => {
            ...
            // 使用新的宠物重新渲染PetUI
            doRender(credential, petList, petStudyStatus, peopleFinder, role);
        });
    });
}
