...
class PersonalPetManagementPageProcessorCastleImpl extends PersonalPetManagementPageProcessor {
    ...
    private peopleFinder?: PeopleFinderComponent;
    ...
    async doInitialize(credential: Credential, context?: PageProcessorContext): Promise<void> {
        ...
        const locationMode = new LocationModeCastle(castleName);
        ...
        this.#petAutoGraze.success = () => {
            doRefresh(credential, this.peopleFinder!);
        };
    }
            ...
            $("#_auto_transfer_pet").on("click", () => {
                if (PageUtils.isColorGrey("_auto_transfer_pet")) {
                    ...
                    if (this.#petAutoTransfer === undefined) {
                        ...
                        this.#petAutoTransfer.success = () => doRefresh(credential, this.peopleFinder!);
                    }
    ...
    doProcessWithPageParsed(credential: Credential, page: PersonalPetManagementPage, context?: PageProcessorContext): void {
        ...
        doProcess(credential, page.petList!, page.petStudyStatus!, this.peopleFinder!);
    }

}

function doProcess(credential: Credential, petList: Pet[], studyStatus: number[], peopleFinder: PeopleFinderComponent) {
    ...
    new PersonalStatus(credential).load().then(role => {
        ...
        new PersonalEquipmentManagement(credential).open().then(page => {
            ...
            doRender(credential, petList, studyStatus, peopleFinder);
            if (cage) {
                $("#openCageButton").show();
...
function doRender(credential: Credential, petList: Pet[], studyStatus: number[], peopleFinder: PeopleFinderComponent) {
    let html = "";
    ...
    // 绑定按钮点击事件处理
    doBind(credential, petList, peopleFinder);

    doBindGrazeButton(credential, peopleFinder);

    if ($("#goldenCageStatus").text() === "on") {
        doRenderGoldenCage(credential, peopleFinder);
    } else {
        // 由于某些bug造成某些号黄金笼子消失
            ...
            $("#takeOutFirstFromCageButton")
                ...
                .on("click", function () {
                    ...
                    NetworkUtils.sendPostRequest("mydata.cgi", request, function (html) {
                        ...
                        doRefresh(credential, peopleFinder);
                    });
                });
        }
    }
    ...
    if ($("#ranchState").text() === "on") {
        doRenderRanch(credential, peopleFinder);
    }

    doRenderPetBorn(credential, petList, peopleFinder);
}
...
function doRenderGoldenCage(credential: Credential, peopleFinder: PeopleFinderComponent) {
    ...
    new GoldenCage(credential).open(index).then(cagePage => {
        ...
        for (const pet of cagePetList) {
            doBindTakeOutButton(credential, pet.index!, peopleFinder);
        }
    });
}

function doBind(credential: Credential, petList: Pet[], peopleFinder: PeopleFinderComponent) {
    for (let i = 0; i < petList.length; i++) {
        const pet = petList[i];
        let buttonId = "pet_" + pet.index + "_uninstall";
        if (!$("#" + buttonId).prop("disabled")) {
            doBindPetUninstallButton(credential, buttonId, peopleFinder);
        }
        buttonId = "pet_" + pet.index + "_install";
        if (!$("#" + buttonId).prop("disabled")) {
            doBindPetInstallButton(credential, buttonId, pet, peopleFinder);
        }
        buttonId = "pet_" + pet.index + "_cage";
        if (!$("#" + buttonId).prop("disabled")) {
            doBindPetCageButton(credential, buttonId, pet, peopleFinder);
        }

        doBindPetSpellButton(credential, pet, peopleFinder);

        buttonId = "pet_" + pet.index + "_love";
        if (!$("#" + buttonId).prop("disabled")) {
            doBindPetLoveButton(credential, buttonId, pet, peopleFinder);
        }
        buttonId = "pet_" + pet.index + "_league";
        if (!$("#" + buttonId).prop("disabled")) {
            doBindPetLeagueButton(credential, buttonId, pet, peopleFinder);
        }
        buttonId = "pet_" + pet.index + "_rename";
        if (!$("#" + buttonId).prop("disabled")) {
            doBindPetRenameButton(credential, buttonId, pet, peopleFinder);
        }

        buttonId = "pet_" + pet.index + "_consecrate";
        if (!$("#" + buttonId).prop("disabled")) {
            doBindPetConsecrateButton(credential, buttonId, pet, peopleFinder);
        }

        buttonId = "pet_" + pet.index + "_send";
        if (!$("#" + buttonId).prop("disabled")) {
            doBindPetSendButton(credential, buttonId, pet, peopleFinder);
        }
    }

    // 设置宠物技能学习位的按钮行为
    doBindPetStudyButton(credential, peopleFinder);

    // 设置查找发送对象按钮行为
    peopleFinder.bindButtons();

    // 设置更新按钮行为
    doBindRefreshButton(credential, peopleFinder);

    // 设置黄金笼子按钮的行为
    doBindGoldenCageButton(credential, peopleFinder);
}

function doRefresh(credential: Credential, peopleFinder: PeopleFinderComponent) {
    new PersonalPetManagement(credential).open().then(petPage => {
        ...
        // 使用新的宠物重新渲染PetUI
        doRender(credential, petList, petStudyStatus, peopleFinder);
    });
}

function doBindPetUninstallButton(credential: Credential, buttonId: string, peopleFinder: PeopleFinderComponent) {
...
function doBindPetInstallButton(credential: Credential, buttonId: string, pet: Pet, peopleFinder: PeopleFinderComponent) {
    $("#" + buttonId).on("click", function () {
        const request = credential.asRequest();
        // @ts-ignore
...
function doBindPetCageButton(credential: Credential, buttonId: string, pet: Pet, peopleFinder: PeopleFinderComponent) {
    $("#" + buttonId).on("click", function () {
        const request = credential.asRequest();
        // @ts-ignore
        request["select"] = pet.index;
...
function doBindPetSpellButton(credential: Credential, pet: Pet, peopleFinder: PeopleFinderComponent) {
...
function doBindPetLoveButton(credential: Credential, buttonId: string, pet: Pet, peopleFinder: PeopleFinderComponent) {
    $("#" + buttonId).on("click", function () {
...
function doBindPetLeagueButton(credential: Credential, buttonId: string, pet: Pet, peopleFinder: PeopleFinderComponent) {
    $("#" + buttonId).on("click", function () {
        const request = credential.asRequest();
        // @ts-ignore
        request["select"] = pet.index;
...
function doBindPetRenameButton(credential: Credential, buttonId: string, pet: Pet, peopleFinder: PeopleFinderComponent) {
    $("#" + buttonId).on("click", function () {
...
function doBindPetConsecrateButton(credential: Credential, buttonId: string, pet: Pet, peopleFinder: PeopleFinderComponent) {
    $("#" + buttonId).on("click", function () {
...
function doBindPetSendButton(credential: Credential, buttonId: string, pet: Pet, peopleFinder: PeopleFinderComponent) {
    $("#" + buttonId).on("click", function () {
        const receiver = peopleFinder.targetPeople;
...
function doBindPetStudyButton(credential: Credential, peopleFinder: PeopleFinderComponent) {
...
function doBindRefreshButton(credential: Credential, peopleFinder: PeopleFinderComponent) {
...
function doBindGoldenCageButton(credential: Credential, peopleFinder: PeopleFinderComponent) {
...
function doBindTakeOutButton(credential: Credential, index: number, peopleFinder: PeopleFinderComponent) {
...
function doRenderRanch(credential: Credential, peopleFinder: PeopleFinderComponent) {
    new CastleRanch(credential).enter().then(status => {
        ...
        doBindSummonButton(credential, peopleFinder);
    });
}

function doBindGrazeButton(credential: Credential, peopleFinder: PeopleFinderComponent) {
    $(".grazeButton")
        .filter((idx, button) => {
...
function doBindSummonButton(credential: Credential, peopleFinder: PeopleFinderComponent) {
    $(".summonButton")
        .on("click", event => {
            const buttonId = $(event.target).attr("id")!;
...
function doRenderPetBorn(credential: Credential, petList: Pet[], peopleFinder: PeopleFinderComponent) {
    new PersonalPetEvolution(credential).open().then(evolutionPage => {
        ...
        if (evolutionPage.malePetList!.length > 0 && evolutionPage.femalePetList!.length > 0) {
            ...
            doBindPropagateButton(credential, evolutionPage, peopleFinder);
        }
        ...
        if (evolutionPage.evolutionPetList!.length > 0) {
            ...
            doBindEvolutionButton(credential, evolutionPage, peopleFinder);
        }
        ...
        if (evolutionPage.degradationPetList!.length > 0) {
            ...
            doBindDegradationButton(credential, evolutionPage, peopleFinder);
        }
        ...
        if (evolutionPage.consecratePetList!.length > 0) {
            ...
            doBindConsecrateButton(credential, evolutionPage, peopleFinder);
        }
    });
}

function doBindPropagateButton(credential: Credential, page: PersonalPetEvolutionPage, peopleFinder: PeopleFinderComponent) {
...
function doBindEvolutionButton(credential: Credential, page: PersonalPetEvolutionPage, peopleFinder: PeopleFinderComponent) {
...
function doBindDegradationButton(credential: Credential, page: PersonalPetEvolutionPage, peopleFinder: PeopleFinderComponent) {
...
function doBindConsecrateButton(credential: Credential, page: PersonalPetEvolutionPage, peopleFinder: PeopleFinderComponent) {
