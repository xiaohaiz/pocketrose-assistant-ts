...
import {DayRange, MonthRange, WeekRange} from "../../util/PocketDateUtils";
...
class PersonalStatisticsPageProcessor extends StatefulPageProcessor {
    ...
    private async generateHTML() {
        ...
        // 恢复之前的div
        const rollCallForm = $("#rollCallForm");
        rollCallForm.after($("" +
        ...
        html += "<tr>";
        html += "<td>";
        html += PocketPage.generatePageHeaderHTML("＜＜ 统 计 报 告 ＞＞", this.roleLocation);
        html += "</td>";
        html += "</tr>";
        html += "<tr>";
    ...
    private async bindButtons() {
        $("#_pocket_page_extension_0").html(() => {
            const formGenerator = new PocketFormGenerator(this.credential, this.location);
            return formGenerator.generateReturnFormHTML();
        });
...
function doBindReport7() {
    $("#report-7").on("click", () => {
        $(".reportButton").prop("disabled", true);
        const includeExternal = $("#includeExternal").prop("checked") as boolean;
        new PowerGemFuseReportGenerator().generate(includeExternal).then(() => {
            $(".reportButton").prop("disabled", false);
        });
    });
}
