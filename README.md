...
1. 装备管理组件增加状态栏。
2. 修复在装备页面祭奠后有可能不从十二宫切换战斗场所的问题。
3. 个人面板集成技能管理组件。
4. 个人面板集成装备管理组件。
5. 修正分身试管页面不出现“获取新分身”的问题。

### 版本 4.6.4

1. 技能管理组件化。
2. 装备管理组件化（仅限随身装备）。
3. 职业管理界面集成了装备管理组件，便于转职后及时换装。
4. 物品商店自动卖出龙珠后加上自动存钱。
5. 城堡仓库装备列表加上排序。
...
1. 宠物页面使用新的找人组件。
2. 装备页面使用新的找人组件。
3. 装备页面中宝石相关操作同时包括龙珠的支持（非传输）。
4. 物品商店全新改版（底层结构优化）。
5. 物品商店提供自动卖出龙珠的功能。

### 版本 4.6.2

1. 个人面板全新升级，基于全新的底层结构改进。
   1. 可跳转到商店、宠物、装备、职业。
   2. 点击头像及分身头像可设置是否定型的标识。
   3. 可切换技能。
   4. 装备相关增强，增加使用、入袋、套装等功能。
   5. 宠物相关增强，增加使用、入笼、十二宫战斗伴侣等功能。
   6. 可切换分身。
   7. 退出个人面板时会触发宠物、装备、智能战斗等。
2. 原生个人状态页面简化，切换分身功能取消（移步个人面板）。
3. 修复装备管理中自动扫描宝石入袋或者入库功能缺少提前刷新的问题。

### 版本 4.6.1

1. 修复非城堡模式的装备管理页面出现“放入仓库”按钮的问题。
2. 装备管理中增加“巧克力套装”的快捷装载按钮。
3. 装备管理中增加“恢复最后使用装备”的功能，能自动恢复上次退出装备页面时使用的装备（方便转职或者切换分身后快速换装）。
4. 装备管理中通用命令按钮布局移动到右上角。
5. 增加装备资料数据，可作为支持更多精准判断的基础。
6. 本次升级涉及到数据库版本变更，如果有问题重启浏览器即可解决。

### 版本 4.6.0

1. 装备管理全新改版，姑且算作3.0版本吧。
2. 外观上变化不大，主要是底层结构的全新结构，目标是性能的提升和交互的合理。
3. 套装的设置从设置页面迁移到装备管理，点击左上角角色头像激活（PC版双击、手机版单击）。
4. 套装新的设置在齐心装备数据结构上有所变化，可能会导致之前的设置数据出错，自行手工重设。
5. 城市里发送装备前和入袋前会尝试主动修复装备。
...
1. 回魂丹不算做闲置装备。
2. 分身试管页面改造完成，并且可点击分身头像设置是否定型的标记。
3. 切换分身之前恢复补满，分身列表能看着舒心一点。
4. 进入个人天真页面时，如果有继续下一场按钮则尝试自动点击。
5. 自动进城时触发智能战斗场所切换（比如离开枫丹后不再选择十二宫）。
